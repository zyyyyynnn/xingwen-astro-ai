...
| M1 开发基线 | P0 | Docker Compose + Web 页面 + FastAPI + Mock 数据 + CI 卡口 | `web/api/postgres` 可一键启动，Web 端可展示完整 Mock 工作流，PR 可拦截依赖漂移 |
| M2 核心功能 | P1 | 数据主链路 + 论文获取与文献总结 + 跨文献推理与学术图谱 | 真实数据进入 CSV、论文候选可展示、推理关系绑定证据、图谱边绑定 evidence 和 trace |
| M3 反馈与交付 | P2 | 缓存兜底、反馈修正、公网 Demo、材料交接 | 演示链路稳定，可交付截图和录屏素材 |
...
M2 包含三个子阶段，按依赖关系串行推进，但同一子阶段内 A/B/C/D 可并行。
