...
M1 暂不引入 Redis、Celery、MinIO、Nginx、RabbitMQ。任务链路先由 FastAPI、数据库状态机和 BackgroundTasks 支撑。
...
| 数据库 | Supabase Postgres / Neon / 阿里云 RDS | 生产 Demo 优先托管 Postgres；本地使用 Docker Compose |
...
`VITE_` 变量会进入前端构建产物，只能放非敏感配置。`PAPER_SOURCE_API_KEY` 如当前论文来源不需要鉴权，可留空或不配置；不得写入前端构建变量。
