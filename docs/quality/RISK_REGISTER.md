...
| R-10 | Medium   | 数据 crossmatch 或单位规则错误          | 匹配冲突率高、单位不可转换、误差/limit 丢失                | C-08 版本化 RuleSet/Benchmark；C-04 Manifest-driven mapping、IAU-provenanced ConversionCatalog、Decimal 策略、raw/canonical 双保留、Transformation Evidence 与冲突保留 | C-08 Benchmark 与 C-04 mapping/conversion/Publisher-port 回归持续通过 |
