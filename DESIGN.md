...
| Implementation | A-01 runtime、A-03 Port 绑定 UI 与 X-01 真实集成 Current；A-02 视觉系统 Pending |
...
设计实现必须由对应 Issue 驱动。A-01 只证明 Astro/React 运行时、路由和共享包边界；A-03 已以同一组件路径消费 Fixture / HTTP Repository Port，并通过真实 Browser/Compose 覆盖 Draft、Contract、Run/Event、Workspace 冲突与刷新恢复、匿名冻结 Share 和撤销。A-02 视觉系统与 M2 科研能力仍按各自 Issue 和运行证据推进。
