...
`app.contracts.v2` 仅用于生成完整目标 OpenAPI，不直接挂载到运行应用。Session 已由运行 Router 实现；WorkspaceSnapshot 与 ShareSnapshot application Router 已实现但在生产事实源接入前不挂载；Project/Run/Artifact 其余运行路由、跨进程持久化和 Workflow 仍由后续 Issue 实现。
...
| `/api/v2` Pydantic / OpenAPI | 七个核心资源及 Workspace/Share Contract Implemented；Session runtime Current，Workspace/Share runtime integration 与其余运行 API Pending |
