...
| R-10 | Medium   | 数据 crossmatch、实体投影或单位规则错误 | 宿主行混入行星字段、assertion 被折叠、匹配冲突率高、单位不可转换、误差/limit 丢失 | C-08 版本化 RuleSet/Benchmark；C-04 冻结 row-grain projection、policy registry、SourceCollectionMember、closed-world 引用、IAU-provenanced ConversionCatalog、集合 Decimal tolerance 与容量边界 | C-08 Benchmark 与 C-04 public mapping/tamper/closed-world/Publisher-port 回归持续通过 |
