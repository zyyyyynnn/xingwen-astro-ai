...
| 岗位 | 负责目录 | 核心职责 | 必须交付 |
| --- | --- | --- | --- |
| A 前端与产品流程 | `apps/web` | 工作流、数据页、文献页、图谱页、反馈入口、UI 规范落地 | 可录屏页面、状态处理、联调说明 |
| B 后端与任务编排 | `apps/api`, `packages/schemas` | FastAPI、任务状态、Schema、Qwen Client、缓存、导出 | API、OpenAPI、统一错误、验证样例 |
| C 数据分析与数据源 | `services/data_pipeline`, `samples/outputs` | 字段清单、数据源、清洗、单位、质量评分、导出 | CSV、字段字典、来源记录、质量评分 |
| D 文献总结与学术图谱 | `services/paper_pipeline`, `services/graph_pipeline` | 文献清单、结构化总结、证据链、Graph JSON | PaperSummary、Graph JSON、证据映射 |
...
| 图谱 | Graph JSON 的边全部绑定 `evidence_ids` |
...
- 数据表、证据面板、图谱、任务时间线必须优先清晰可读，不为装饰牺牲可信度。
- UI 改动需同步检查 `docs/quality/REVIEW_CHECKLIST.md` 中的 UI 项。

## 7. 科研可信红线

- 前端不直连 Qwen，不直连外部天文数据源。
- API Key 只允许在后端环境变量或部署平台 Secrets 中存在。
- 模型输出必须结构化校验，不能直接当作事实。
- 展示字段、结论、图谱边必须能追溯到 `Evidence`。
...
禁止宣传：任意天文方向、任意 PDF 全文解析、任意图表全自动解析、无边界 AI Scientist。