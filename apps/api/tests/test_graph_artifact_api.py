...
def test_graph_cursor_deterministic_round_trip_without_delimiter_ambiguity(
    read_fixture: GraphReadFixture,
    ...
    from app.services.graph_artifacts import _decode_cursor, _encode_cursor

    test_key_str = "test-key-9"
    monkeypatch.setattr(
        settings.CURSOR_SIGNING_KEY,
        "get_secret_value",
        lambda: test_key_str,
    )
    key = test_key_str.encode("utf-8")

    version_id = read_fixture.graph_version_id
    collection = "nodes"
    filters = {"node_type": None}
    legacy_ordering = "stable_id.asc.v1.0"

    def legacy_unsigned_payload(last_id: str) -> bytes:
        ...
        return json.dumps(
            {"scope": legacy_scope, "last_id": last_id}, separators=(",", ":")
        ).encode("utf-8")

    fixture_node_ids = tuple(node.node_id for node in read_fixture.candidate.nodes)
    fallback_last_ids = tuple(f"node.legacy.{index}" for index in range(4096))
    last_id = next(
        candidate
    ...
    legacy_payload = legacy_unsigned_payload(last_id)
    legacy_raw_digest = hmac.new(key, legacy_payload, hashlib.sha256).digest()

    assert b"." in legacy_raw_digest, (
        "Legacy raw SHA-256 digest must contain 0x2e for regression proof; "
        f"ordering={legacy_ordering}, digest={legacy_raw_digest.hex()}"
    )

    legacy_raw = legacy_payload + b"." + legacy_raw_digest
    legacy_payload_part, legacy_sig_part = legacy_raw.rsplit(b".", 1)
    assert legacy_sig_part != legacy_raw_digest, (
        "Legacy rsplit MUST produce an incorrect signature because the raw "
        "digest contains 0x2e"
    )
    assert legacy_payload_part != legacy_payload

    encoded_cursor = _encode_cursor(
        version_id=version_id,
        collection=collection,
        filters=filters,
        last_id=last_id,
    )
    decoded_last_id = _decode_cursor(
        encoded_cursor,
        version_id=version_id,
        collection=collection,
        filters=filters,
    )
    assert decoded_last_id == last_id

    if last_id in fixture_node_ids:
        client = _client(read_fixture)
        response = client.get(
            _graph_path(read_fixture, f"/nodes?cursor={encoded_cursor}")
        )
        assert response.status_code == 200
