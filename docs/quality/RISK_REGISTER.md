...
| R-10 | Medium   | 数据 crossmatch 或单位规则错误          | 匹配冲突率高、单位不可转换、误差/limit 丢失                | C-08 版本化 RuleSet、版本化 Benchmark（`crossmatch-benchmark.v1.json`）、显式审查绑定、冲突保留与双方 Evidence | C-08 版本化 Benchmark 持续通过         |
