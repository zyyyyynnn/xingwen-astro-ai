...
- Crossmatch-backed 数据 revision 在既有 `cleaning_data` 服务内按冻结 input 复用可验证 upstream：Quality-only exact readmit candidate 后仅重跑质量；mapping/unit 复用 acquisition 与 Crossmatch；crossmatch policy 变化复用 acquisition；只有 Plan 含 `fetching_data` 时才允许重新 acquisition。自然语言 Feedback 不是数据 patch DSL，无法由现有结构化 source/policy 表达的变化稳定返回 replan-required。SourceTable/Gaia revision 继续由原 scientific step 生产，只共享 replay 与 frozen publication target 校验。
