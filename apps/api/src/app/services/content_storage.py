...
class ContentStorage(Protocol):
    ...
    async def store_stream(
        ...
        *,
        ...
        """Persist a bounded producer stream without materializing it in memory."""
...
class LocalContentStorage:
    ...
    async def store(self, content: bytes, content_hash: str) -> str:
        ...
        return await self.store_stream(
            chunks(), content_hash, expected_size=len(content)
        )
        ...
        *,
        ...
        try:
            ...
            if self._verify_existing(blob, content_hash) is True:
                return _storage_ref(content_hash)
            self._repair_corrupt(blob, content_hash, temp_blob)
            return _storage_ref(content_hash)
        finally:
    ...
    async def _write_temp_stream(
        ...
        *,
        ...
        if expected_size is not None and size_bytes != expected_size:
            raise ValueError(
                f"content size {size_bytes} does not match expected_size {expected_size}"
            )
    ...
    def _verify_existing(blob: Path, content_hash: str) -> bool | None:
        ...
        try:
            actual_hash, _, _ = _hash_file(blob)
        except FileNotFoundError:
            return False
        except (PermissionError, OSError) as exc:
            raise ContentStorageError(
    ...
    def _repair_corrupt(
        self, blob: Path, content_hash: str, temp_blob: Path
    ) -> None:
        ...
        quarantine = (
            blob.parent / f".corrupt_{content_hash[7:23]}_{secrets.token_hex(8)}"
        )
        try:
        ...
        try:
            if not _publish_no_replace(temp_blob, blob):
            ...
            # Preserve the last known bytes when publication itself fails.  A
            # maintenance audit can report corruption, but store() must never
            # turn it into silent data loss.
            if quarantine.is_file() and not blob.exists():
                os.replace(quarantine, blob)
            raise
        else:
