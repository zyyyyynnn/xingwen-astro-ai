...
- Worker 在取得 lease 后、创建任何 ProducerExecution 或调用 provider/source 前，通过 Confirmation 的 exact `run_id` 装载 RevisionPlan，验证 Run derivation/parent/Contract、重算 plan hash，并校验全部 frozen latest 指针。预检失败通过既有 Workflow Store 将首 Step 与 Run 原子标记 failed；执行中不回退 latest、不自动 rebase 或生成新 Plan。
- confirmed Plan 没有 data-side recompute decision 时，Worker 仍读取 frozen replay input 并完成 Contract、Manifest、policy、provenance 与 candidate cross-binding 校验；验证通过后只保留 data reuse decision，不构建 candidate、不创建数据 ProducerExecution 或新版本。数据三件套只能整体 reuse 或整体 recompute；混合决策要求重新生成 Plan。
- Crossmatch-backed 数据 revision 在既有 `cleaning_data` 服务内按冻结 input 复用可验证 upstream：Quality-only 优先 exact readmit candidate 后仅重跑质量，reuse 无效但 Plan 已授权 `cleaning_data` 时执行完整 Data Artifact build 与 Data Quality；mapping/unit 复用 acquisition 与 Crossmatch；crossmatch policy 或 source identity 变化必须进入 normal/revision 共用的 repair checkpoint seam，并以新 Crossmatch 重新执行 document admission；只有 Plan 含 `fetching_data` 时才允许 Crossmatch source acquisition，且 canonical `source_input_hash` 未变时不重复 Crossmatch。未重采时新版本保留已验证 baseline provenance，重采时使用实际 acquisition provenance，单一 `source_mode` 无法真实表达时拒绝发布。自然语言 Feedback 不是数据 patch DSL。SourceTable/Gaia revision 继续由原 scientific step 生产：persisted raw cells 足够时按 current mapping、unit 与 quality policy 重新 admission，不调用 provider；authority 不足时在 provider 前返回 replan-required。结构化 source authorization 才调用既有 adapter。AnalysisReport 与 data decision 分别执行，同一步的 report provider output 只有在 source reacquisition 获授权时才能作为 data input。
