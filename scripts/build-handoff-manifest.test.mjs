...
test("release candidate enforces retries=0 and full-text revision budget", () => {
  const root = process.cwd();
  const scriptContent = readFileSync(
    path.join(root, "scripts", "run-release-candidate.mjs"),
    "utf8",
  );
  assert.match(
    scriptContent,
    /"--retries",\s*"0"/,
    "release candidate must pass --retries 0 to prevent wasteful test-level full-chain retries",
  );
  ...
  const specContent = readFileSync(
    path.join(
      ...
      "tests",
      "e2e-integration",
      "release-candidate-live.spec.ts",
    ),
    "utf8",
  );
  assert.match(
    specContent,
    /FULL_TEXT_REVISION_TIMEOUT_MS\s*=\s*35\s*\*\s*60_?000/,
    "document revision must have a 35 min observation budget",
  );
  assert.match(
    specContent,
    /waitForRun\(\s*page,\s*documentRunId,\s*FULL_TEXT_REVISION_TIMEOUT_MS,?\s*\)/,
    "only documentRunId must use the 35 min revision budget",
  );
  assert.match(
    specContent,
    /async function waitForRun\(\s*page:\s*Page,\s*runId:\s*string,\s*timeout\s*=\s*20\s*\*\s*60_?000,?\s*\)/,
    "default waitForRun timeout must remain 20 min",
  );
});
