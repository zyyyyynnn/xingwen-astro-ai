"""Validate a produced D-10 native benchmark report artifact (fail-closed).

Checks that the benchmark genuinely ran with no silent skips:
- the report parses as a D-10 ``BenchmarkReport``;
...
def main() -> int:
    if len(sys.argv) < 2:
        print("usage: check_d10_benchmark_report.py <report.json>", file=sys.stderr)
        return 2
    ...
    if errors:
        ...
        return 1
    print(
        f"D-10 benchmark report valid: {len(report.cases)} cases, "
