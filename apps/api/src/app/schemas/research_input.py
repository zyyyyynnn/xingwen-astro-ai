...
Parquet, JSON, FITS, image datasets, images and plain-text inputs. This contract only *receives* inputs into an
...
class ResearchInputCreate(BaseModel):
    ...
    the file types (pdf/csv/xlsx/parquet/json/image/image_dataset/fits) always arrive through the multipart
...
#: ``application/json``; pdf/csv/xlsx/parquet/json/image/image_dataset/fits are multipart-only. The public name
