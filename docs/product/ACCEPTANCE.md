...
- 当前 Docker Compose 可启动 `web`、`api`、`postgres`；
- `/api/v1` 健康检查和回归测试通过；
- pnpm、uv、lockfile、Schema 导出和 Foundation CI 通过；
- 旧前端没有新增目标业务功能。
...
- X-00 已冻结主案例事实包；
- Astro Site、React Workspace 和共享包空基线可独立构建；
...
- Current、Target 和 Pending 在页面、文档和材料中无混淆。
