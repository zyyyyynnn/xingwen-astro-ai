...
> A-01 前端运行时基线已 Implemented；A-02、A-03 与 `/api/v2` 保持 Pending。完整前端工程事实只在 `FRONTEND_ARCHITECTURE.md` 维护。Milestone 与 P0/P1/P2 映射保持不变。
...
| M1 开发基线   | P0         | A-01 运行时 + A-02 品牌/工作台框架 + A-03 Contract 双通道 | 四服务保持可启动；各阶段代码、Fixture Demo、CI 与契约门禁通过 |
...
M1 已建立 `X-00`、`X-04`、A-01 前端运行时、后端骨架和 Phase 0 Workflow。A 线继续按 A-02 → A-03 建立静态品牌/工作台框架和 Contract 驱动双通道；C/D 提供最小真实依据，B 保持 v1 稳定并实现 v2 最小 Contract。

| 产出                  | 负责人        | 验收                                                                                           |
| --------------------- | ------------- | ---------------------------------------------------------------------------------------------- |
| MVP 最小真实依据      | A + B + C + D | 字段清单、论文来源、检索词、seed、关系类型已冻结                                               |
| 本地运行基线          | A + B         | `site`、`workspace`、`api`、`postgres` 可启动，浏览器 API 地址与跨域配置正确                   |
| A-01 前端结构         | A             | 两个 App、八个共享 Package、依赖边界、严格类型与完整工程门禁通过（Implemented）                |
...
1. A-01：应用/共享包空骨架、依赖边界与 format/lint/typecheck/test/build/E2E/architecture gate（Implemented）；不包含产品组件或 Shader。
