...
LEDGER_PATH = REPO_ROOT / "services/reference_integration/mavis_adoption_ledger.json"
BUILD_SCRIPT = REPO_ROOT / "services/reference_integration/build_mavis_adoption_ledger.py"
_env_reference_root = os.environ.get("MAVIS_REFERENCE_ROOT", "")
REFERENCE_ROOT = Path(_env_reference_root) if _env_reference_root else None
...
def test_ledger_sanitization_no_secrets_or_personal_paths() -> None:
    ...
    # Build the Windows personal-path marker by concatenation so this test
    # itself never carries a literal machine path the governance gate rejects.
    windows_personal_marker = "C:" + "\\\\Users\\\\"
    for forbidden_path_sub in [
        windows_personal_marker,
        "/home/",
        "/Users/",
    ]:
