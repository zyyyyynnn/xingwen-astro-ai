"""Stable logical identities owned by the Versioned Evidence Graph pipeline.
    ...
    def __post_init__(self) -> None:
        ...
        if self.node_type not in _GENERATED_NODE_TYPES:
            raise GraphIdentityError(
                f"Versioned Evidence Graph does not generate {self.node_type.value!r} nodes"
            )
...
class GraphEdgeIdentity:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.edge_type) is GraphEdgeType:
            ...
            if expected is None:
                raise GraphIdentityError(
                    f"{self.edge_type.value!r} is not a Versioned Evidence Graph structural edge"
                )
...
def supports_finding_edge_identity(
    ...
    """Create the Versioned Evidence Graph paper -> claim supports_finding edge."""
...
def graph_edge_type_for_literature_relation(
    ...
    value match.  Versioned Evidence Graph admission turns this failure into ``taxonomy_violation``.
    """
    ...
    try:
        ...
        raise GraphIdentityError(
            f"{relation_type.value!r} is outside the Versioned Evidence Graph GraphEdgeType taxonomy"
        ) from exc
