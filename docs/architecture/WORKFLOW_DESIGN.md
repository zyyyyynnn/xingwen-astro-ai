...
- confirmed Plan 没有 data-side recompute decision 时，Worker 只把 frozen data ArtifactVersion 作为 reuse identity 放入上下文，不读取 replay input、不构建 candidate、不创建数据 ProducerExecution 或新版本。数据三件套只能整体 reuse 或整体 recompute；混合决策要求重新生成 Plan。
- Crossmatch-backed 数据 revision 在既有 `cleaning_data` 服务内按冻结 input 复用可验证 upstream：Quality-only 优先 exact readmit candidate 后仅重跑质量，reuse 无效但 Plan 已授权 `cleaning_data` 时执行完整 Data Artifact build 与 Data Quality；mapping/unit 复用 acquisition 与 Crossmatch；crossmatch policy 或 source identity 变化必须进入 normal/revision 共用的 repair checkpoint seam，并以新 Crossmatch 重新执行 document admission；只有 Plan 含 `fetching_data` 时才允许 Crossmatch source acquisition，且 canonical `source_input_hash` 未变时不重复 Crossmatch。自然语言 Feedback 不是数据 patch DSL。SourceTable/Gaia revision 继续由原 scientific step 生产：quality-only 在 provider 前执行 replay 与 Data Quality，结构化 source authorization 才调用既有 adapter，unsupported selective stage 在 provider 前 fail closed，Data Artifact 必须携带显式 frozen revision publication targets，AnalysisReport 仍由 scientific owner 按 Plan decision 处理。
