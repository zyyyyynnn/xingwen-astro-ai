...
| 012 | Superseded by ADR-021 | Vue / shadcn-vue 仅为当前迁移源 |
| 013 | Accepted for current local baseline | Docker Compose 保持当前本地入口 |
...
| 021–028 | Accepted for implementation | 目标前端、领域、安全和视觉方案 |
...
**Historical decision:** Phase 0 使用 Vue、Vite、shadcn-vue、Pinia、Vue Router 和 Vue Flow 建立可运行骨架。

**Reason retained:** 该方案验证了本地构建、Compose 和 v1 API 边界。

**Replacement:** 新业务迁移到 Astro Brand Site、React Workspace 和共享包；旧 `apps/web` 只作短期回退。

## ADR-013：Docker-first 本地基线

**Status:** Accepted for current local baseline

**Decision:** 当前本地统一使用 `web`、`api`、`postgres` Compose 服务及锁定运行时。

**Rationale:** 降低成员本机版本和网络差异。

**Consequence:** 目标前端迁移由独立 Issue 更新 Compose；本文不把目标拓扑写成已运行。
...
**Consequence:** `packages/schemas` 是当前过渡产物，目标由生成 Contract 包消费。
...
**Decision:** pnpm Monorepo；`apps/site` 使用 Astro 静态输出，`apps/workspace` 使用 React；共享 design tokens、UI、visual engine、domain、contracts、data access、workspace core 和 testing。

**Consequences:** 职责分离且利于未来 Tauri；代价是一次性迁移和短期双构建。

**Rejected:** 继续扩展单体 Vue；无明确需求的全站 Next.js；用 Astro Islands 承载完整工作台。
