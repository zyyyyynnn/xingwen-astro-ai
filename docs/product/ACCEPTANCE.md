...
> A-01 Astro/React runtime 与 FastAPI `/api/v1` 是 Current；A-02、A-03 与 `/api/v2` 为 Pending。不能用文档通过代替代码和运行证据。前端工程事实以 [FRONTEND_ARCHITECTURE.md](../architecture/FRONTEND_ARCHITECTURE.md) 为准。
...
| Issue | 最低产物                                                                                                                                                           |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| A-01  | 两个 App、八个共享 Package、依赖边界、strict typecheck、format/lint/test/build/E2E/architecture gate、Compose 与零残留检查通过；无产品组件或 Shader（Implemented） |
...
- 当前 `/api/v1` 保持兼容；v2 不以修改 v1 响应伪装实现。
