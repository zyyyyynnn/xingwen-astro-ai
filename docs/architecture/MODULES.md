...
- Workflow 是 Run/Step/Attempt/Event/lease/cancel/retry/cache/publish 的唯一编排边界；不得创建第二套执行器或状态机。
...
- **职责**：提供统一无版本 `/api/*` 接口，管理 Session、Project、Contract、Run 状态机与 Event 读取，处理并发锁、幂等、重试、取消、CacheSelector、Feedback 与持久化事务，并通过一个真实 Workflow Executor 连接 Step Adapter。
