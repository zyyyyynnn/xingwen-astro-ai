"""Deterministic A-06 demo fixture built by the real D-03 pipeline.
...
TypeScript. It reuses the deterministic A-05 ``build_demo_collection()`` as
the D-02 provenance input, drives the real ``PaperSummaryPipeline.admit``
...
admitted ``PaperSummaryArtifactContent`` through the same read shape B-07
...
SUMMARY_PROVENANCE_NOTE = (
    "Demo Replay fixture generated by the real D-03 PaperSummaryPipeline "
    "over the deterministic A-05 demo PaperCollection (frozen benchmark "
...
def build_demo_summary() -> PaperSummaryArtifactContent:
    """Run the real D-03 admission against the real A-05 demo collection."""
...
def build_demo_summary_read() -> tuple[PaperSummaryRead, ArtifactVersionDetail]:
    """Project the admitted summary through the B-07 read shape."""
    ...
    # B-07 re-links summary snapshot references to persisted snapshots by the
    ...
    # summary source_record_id inside the locator (B-07 cross-link check).
    ...
    # same canonical PaperSummary dump: B-07 recomputes content_hash over
