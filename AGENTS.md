...
本文件是仓库内自动化实现 Agent 的执行宪法。它定义工作前置、范围、Git 安全、架构纪律、产品完成定义、验证与正式 Review 规则。产品、架构、数据、UX 等领域事实由 [docs/README.md](docs/README.md) 指向的唯一 Authority 定义。
...
Authority 与实现冲突时不得选择更方便的一方继续。先判定实现漂移、文档漂移，或本次任务是否正在改变 Authority，再把规则与实现重新收敛为单一事实源。
...
- 检查目标能力是否已经存在，先做 capability gap，再决定新增、迁入、合并或删除；
...
- 不为假设的未来需求建立通用 registry、strategy、planner、audit framework 或迁移框架；
...
- 外部能力的具体名称、仓库地址、版本、提交号、迁移矩阵与差异审查只存在于实施 Prompt、PR/Review 或必要 provenance，不进入治理 Markdown。
