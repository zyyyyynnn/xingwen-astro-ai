...
if (!lock) {
  ...
  if (lock.product !== "OpenHands") {
    failures.push(`G1: unique product must be OpenHands, found "${lock.product}".`);
  }
  if (!lock.unique_agent_product_source) {
    failures.push("G1: upstream-lock.unique_agent_product_source must be true.");
  }
  // G2
  if (lock.repository !== "https://github.com/OpenHands/OpenHands.git") {
    failures.push(`G2: repository must be OpenHands/OpenHands, found "${lock.repository}".`);
  }
  if (lock.tag !== "v1.10.0") {
    failures.push(`G2: tag must be v1.10.0, found "${lock.tag}".`);
  }
  if (lock.commit !== "56638693908b8ac83a2fa3bde6eb6c33aae37f4b") {
    failures.push(`G2: commit must be 56638693908b8ac83a2fa3bde6eb6c33aae37f4b, found "${lock.commit}".`);
  }
  if (typeof lock.commit !== "string" || lock.commit.length !== 40) {
    failures.push("G2: commit must be a 40-char SHA.");
  }
  if (["latest", "main", "master", ""].includes(lock.tag) || lock.tag?.startsWith("main")) {
  ...
  if (badLeak.length) {
    failures.push(`G3: losing candidate leaked into lock: ${badLeak.join(", ")}.`);
  }
  if (!Array.isArray(rejected) || rejected.length === 0) {
    notes.push("G3: no rejected_candidates recorded (expected AnythingLLM/LibreChat/Dify).");
  }
  // G6
  for (const cls of lock.adoption_class_forbidden ?? []) {
    if (["REWRITE", "RECREATE", "REIMPLEMENT", "INSPIRED_BY"].includes(cls)) continue;
    failures.push(`G6: adoption_class_forbidden list should only contain REWRITE/RECREATE/REIMPLEMENT/INSPIRED_BY; found "${cls}".`);
  }
...
const forbiddenRoots = ["apps/workspace/vendor", "apps/workspace/third_party", "apps/workspace/upstream", "packages/third_party"];
for (const r of forbiddenRoots) {
  ...
  for (const entry of readdirSync(p)) {
    ...
    failures.push(`G4/G8: competing vendor root detected: ${r}/${entry} (only ${VENDOR_ROOT} allowed).`);
  }
}
...
if (blueprint) {
  for (const rule of blueprint.hard_rules ?? []) {
    if (/rewrite|recreate|reimplement|inspired by/i.test(rule) && !/never|not|forbid|!=/i.test(rule)) {
  ...
  for (const f of forbidden) {
    if (JSON.stringify(blueprint).includes(`"${f}"`) && !JSON.stringify(blueprint.adoption_class_allowlist ?? []).includes(f)) {
      failures.push(`G6: blueprint references forbidden class "${f}" outside allowlist context.`);
    }
  }
} else {
...
if (lock) {
  for (const field of ["repository", "product", "commit"]) {
    ...
    for (const term of leakScan) {
      if (v.includes(term)) {
        failures.push(`G3/G8: losing candidate "${term}" in upstream-lock.${field}.`);
      }
    }
  }
}
for (const f of scope.files ?? []) {
  if (!["REQUIRED_VENDOR", "REQUIRED_TRANSITIVE", "PARTIAL_SURGICAL"].includes(f.classification)) continue;
  const v = String(f.upstream_path ?? "").toLowerCase();
  for (const term of leakScan) {
    if (v.includes(term)) {
      failures.push(`G3/G8: losing candidate "${term}" in adopted upstream_path ${f.upstream_path}.`);
    }
  }
}
  ...
  if (excluded.length === 0) {
    failures.push("G7: source-scope has no EXCLUDED entries (coding surfaces must be excluded).");
  }
  const codingTerms = ["terminal", "diff-viewer", "browser/", "git-service", "vscode", "electron", "cloud"];
  ...
  if (badAdopted.length) {
    failures.push(`G7: coding surface adopted as vendor: ${badAdopted.map((f) => f.upstream_path).join(", ")}.`);
  }
  ...
  if (badClass.length) {
    failures.push(`G6: forbidden adoption_class present: ${badClass.length} file(s).`);
  }
  // G5 (forward): if a src/ dir exists, every file needs a manifest entry
  const srcDir = resolve(root, `${VENDOR_ROOT}/src`);
  if (existsSync(srcDir)) {
    const manifestPaths = new Set((scope.files ?? []).map((f) => f.upstream_path));
    const onDisk = [];
    const walk = (dir) => {
      for (const e of readdirSync(dir)) {
        const full = join(dir, e);
        if (statSync(full).isDirectory()) walk(full);
        else if (/\.(ts|tsx)$/.test(e)) onDisk.push(full.replace(root, "").replace(/\\/g, "/"));
      }
    };
    walk(srcDir);
    if (onDisk.length && onDisk.length !== manifestPaths.size) {
      failures.push(`G5: vendored src files (${onDisk.length}) != manifest entries (${manifestPaths.size}) — 1:1 required.`);
    }
    notes.push(`G5: ${onDisk.length} vendored files cross-checked against manifest.`);
  } else {
    notes.push("G5: no vendored src/ yet (Phase 2 = metadata only) — forward gate armed.");
  }
}
