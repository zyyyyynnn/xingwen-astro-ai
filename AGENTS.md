...
- 分支：从 `main` 新建，命名体现岗位或模块（`feat/a-*`、`feat/b-*`、`feat/c-*`、`feat/d-*`、`feat/x-*`、`fix/*`、`docs/*`），不直接推送 `main`。
- Commit：一个 commit 只表达一个主要目的，前缀 `feat` / `fix` / `docs` / `chore`。
- PR 必须关联 Issue，并在描述中说明：
  - 改动范围。
  - 验证方式与结果（无法验证时说明原因）。
  - 是否改接口、数据结构、UI 基线、部署或安全口径。
  - 是否影响演示截图、导出物或材料交接口径。
- 文档同步：PR 涉及接口、数据、技术栈、模块、启动、范围、风险或安全变化时，必须同步第 6 节对应文档。
- 合并：Squash merge，通过 Review 且分支可合并后合并；`X-05` 完成后 PR 必须通过 CI，不得绕过失败的卡口合并；合并后删除分支。
- 禁止：无明确任务目标的 PR、无验证说明、接口变不改 `API_CONTRACT.md`、数据变不改 `DATA_MODEL.md`、UI 变不改 `DESIGN.md`、暴露密钥、宣传未实现能力。
