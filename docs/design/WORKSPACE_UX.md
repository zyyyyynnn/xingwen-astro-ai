...
当前实现（A-05）：`/workspace` 选中 `paper_collection` 版本后，中央画布渲染论文获取与候选审查面——展示检索 Query/参数、来源执行与失败分类、指标、规则/复现标识（hash）与候选列表；筛选（文本/入选状态/来源/重复与冲突）只隐藏行，不改变服务端稳定排名；入选与排除原因、重复组、不确定字段冲突逐候选展示；候选选择驱动 Provenance Observatory（SourceSnapshot/Evidence），Evidence 打开后进入既有 pin/Share 链路；非 http(s) 外部链接只以纯文本呈现；empty / rate_limited / source_failed / invalid / network_error 均有可执行重试；Fixture 数据明示 Demo Replay 确定性演示标注。当前为只读审查，人工改判/重跑检索属后续 Issue。
