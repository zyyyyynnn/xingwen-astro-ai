...
- 数据产物按完整 Dataset/FieldDictionary/SourceCollection bundle 影响并继续传播到真实下游；PaperCollection 影响 Summary、Claim、Relation、Trace 与 Graph；Summary、Claim、Relation、Trace、Graph 依次只影响自身及其下游。RevisionPlan impact authority 仅从结构化 Feedback、真实 producer topology 与父 Contract canonical closure 冻结 impact：SourceCollection 的 artifact/artifact-version correction/evidence 在 Crossmatch topology 中显式加入 `fetching_data`；同一 Gaia scientific step 的 data quality-only 只标记数据三件套，AnalysisReport 保持 reuse，source reacquisition 才由原 scientific owner 重算其 Plan-authorized output。其他 frozen ArtifactVersion 作为 reuse identity，不复制内容或直接发布新版本。
