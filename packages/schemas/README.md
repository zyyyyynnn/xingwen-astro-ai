...
```powershell
Set-Location apps/api
uv run python ../../scripts/export_schemas.py --output ../../packages/schemas/generated/core --include ResearchProject --include ResearchContractDraft --include ResearchContract --include ResearchRun --include RunEvent --include ResearchArtifact --include ArtifactVersion --include ResearchArtifactDetail --include ArtifactVersionDetail --include PaperSummaryArtifactContent --include PaperSummaryRead --include PaperCollectionRead --include PaperCollectionCandidateRead --include DataArtifactReadBase --include DatasetArtifactRead --include FieldDictionaryArtifactRead --include SourceCollectionArtifactRead --include DataArtifactRowRead --include LiteratureClaimRead --include LiteratureRelationRead --include LiteratureReasoningTraceRead --include CreateArtifactExportRequest --include ArtifactExportRead --include EvidenceRead --include SourceSnapshotDetail --include SessionCreated --include WorkspaceSnapshot --include ShareSnapshot --include ShareSnapshotCreated --include PublicShareSnapshot --check
...
D-08 LiteratureRelation/ReasoningTrace 领域 Pipeline 同样使用独立的已提交漂移基线；它
不是 HTTP DTO：
...
- `generated/literature_relation` 固定 D-08 唯一领域编写源的 extraction、Relation/ReasoningTrace admission、publisher candidate 与 Benchmark report；Phase 0 `reasoning.LiteratureRelation`/`ReasoningTrace` 和 core `LiteratureRelationsArtifactContent`/`ReasoningTracesArtifactContent` 只保留冻结传输/投影兼容，不能进入 Publisher。
- B-08 HTTP 投影直接组合 D-07/D-08 typed candidate 与持久化 Version、Evidence、SourceSnapshot；`LiteratureClaimRead`、`LiteratureRelationRead` 和 `LiteratureReasoningTraceRead` 进入 `generated/core` 与 OpenAPI，不复制 Claim、Relation 或 Trace 领域类型。
...
- D-08 LiteratureRelation/ReasoningTrace Pipeline Schema 使用独立 tracked JSON Schema 基线，不进入 HTTP OpenAPI。
- B-08 Literature Claim/Relation/Trace 读取投影属于 Core HTTP Contract；其嵌套领域字段仍引用 D-07/D-08 Pydantic 编写源。
