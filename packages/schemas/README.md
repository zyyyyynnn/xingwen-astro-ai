...
- `generated/phase0` 同步导出 C-04 的 build input、三类 typed candidate、MappingRuleSet 与 UnitConversionCatalog；`DataArtifactBuildResult` 仅是进程内编排结果，不是公共 JSON Contract。这些 Schema 是 Pipeline Contract，不会因此成为 HTTP DTO 或数据库记录。
