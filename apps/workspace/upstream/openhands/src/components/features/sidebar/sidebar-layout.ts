...
export const SIDEBAR_COLLAPSED_ICON_SLOT_CLASS =
  "relative h-9 min-h-9 w-[18px] shrink-0";

export function sidebarNavListClassName(): string {
  return "flex w-full shrink-0 flex-col gap-1 px-2";
}

export function sidebarNavRowClassName(): string {
  return cn(
    "group flex h-9 min-h-9 min-w-0 items-center gap-2 rounded-[var(--oh-radius-sm)] px-2.5 text-sm",
