...
_TYPE_MIME_FAMILIES: dict[ResearchInputType, tuple[str, ...]] = {
    ...
    ResearchInputType.fits: ("application/fits",),
...
class ResearchInputPolicy:
    ...
    max_size_bytes: int
    image_dataset: ImageDatasetPolicy
...
def validate_declared_mime(
    *,
    ...
    if (
        normalized_client_mime is not None
        and normalized_client_mime != sniffed_mime
    ):
    ...
    if authoritative_mime not in allowed_mimes:
        return None
